...
import { Layers, Plus, Search, Pencil, Trash2, X, ShieldCheck, Users } from "lucide-react";
import { fetchDepartments, upsertDepartment, deleteDepartment } from "../../services/supabaseService";

const EMPTY_FORM = { name: "", code: "", description: "", status: "Active" };

function Toast({ msg }) {
  if (!msg) return null;
  return <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", background: "#1e293b", color: "white", padding: "11px 22px", borderRadius: 12, fontSize: 13, fontWeight: 600, zIndex: 9999, whiteSpace: "nowrap", boxShadow: "0 8px 30px #0003" }}>{msg}</div>;
}

export default function DepartmentPage({ employees = [] }) {
  ...
  useEffect(() => { fetchDepartments().then((d) => { setDepartments(d); setLoading(false); }); }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return departments.filter((d) => {
      ...
      if (!q) return true;
      return [d.name, d.code, d.description].filter(Boolean).join(" ").toLowerCase().includes(q);
    });
  ...
  const save = async () => {
    if (!form.name?.trim()) { showToast("Department name is required."); return; }
    if (!form.code?.trim()) { showToast("Department code is required."); return; }
    if (departments.some((d) => d.code.toLowerCase() === form.code.trim().toLowerCase() && d.id !== editing)) {
      showToast("Department code already exists."); return;
    }
    setSaving(true);
    const row = editing
      ? { ...form, id: editing, updated_at: new Date().toISOString() }
      : { ...form, name: form.name.trim(), code: form.code.trim().toUpperCase() };
    const saved = await upsertDepartment(row);
    setDepartments((prev) => editing ? prev.map((d) => d.id === editing ? saved : d) : [...prev, saved]);
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      <div className="welcome-row">
        ...
        <button className="primary-button" onClick={openAdd}><Plus size={16} /> Add Department</button>
      </div>

      <div className="stats-grid" style={{ marginBottom: 22 }}>
        <div className="stat-card blue"><div className="stat-icon"><Layers size={20} /></div><div className="stat-content"><span>Total Departments</span><strong>{departments.length}</strong><small>Configured</small></div></div>
        <div className="stat-card green"><div className="stat-icon"><ShieldCheck size={20} /></div><div className="stat-content"><span>Active</span><strong>{departments.filter((d) => d.status !== "Inactive").length}</strong><small>Operational</small></div></div>
        <div className="stat-card purple"><div className="stat-icon"><Users size={20} /></div><div className="stat-content"><span>Total Employees</span><strong>{employees.length}</strong><small>Across all departments</small></div></div>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: "white", border: "1px solid #e2e8f0", borderRadius: 11, padding: "0 14px", flex: 1, minWidth: 200 }}>
          ...
          <input style={{ border: 0, outline: 0, padding: "11px 0", fontSize: 13, width: "100%", background: "transparent" }} value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, code, description…" />
        ...
        <span style={{ fontSize: 12, color: "#7b8494", alignSelf: "center" }}>{filtered.length} department{filtered.length !== 1 ? "s" : ""}</span>
      </div>

      <div className="panel" style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 2fr 0.8fr 0.8fr 120px", padding: "11px 20px", borderBottom: "1px solid #f1f5f9", background: "#f8fafc" }}>
          {["Department", "Code", "Description", "Employees", "Status", "Actions"].map((h) => (
            <div key={h} style={{ fontSize: 10, fontWeight: 800, color: "#7b8494", textTransform: "uppercase", letterSpacing: 0.5 }}>{h}</div>
          ))}
        </div>
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "50px 20px", color: "#94a3b8" }}>
            <Layers size={38} style={{ opacity: 0.3, marginBottom: 12 }} />
            <strong style={{ display: "block", color: "#475569", marginBottom: 4 }}>No departments found</strong>
            <span style={{ fontSize: 12 }}>{search ? "Try a different search term." : "Click \"Add Department\" to create one."}</span>
          </div>
        ) : filtered.map((d, idx) => (
          <div key={d.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 2fr 0.8fr 0.8fr 120px", padding: "14px 20px", borderBottom: idx < filtered.length - 1 ? "1px solid #f8fafc" : "none", alignItems: "center" }}
            onMouseEnter={(e) => e.currentTarget.style.background = "#f8fafc"}
            onMouseLeave={(e) => e.currentTarget.style.background = "white"}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 38, height: 38, borderRadius: 11, background: "#f0fdf4", color: "#15803d", display: "grid", placeItems: "center", fontWeight: 800, fontSize: 15, flexShrink: 0 }}>{d.name.charAt(0)}</div>
              <strong style={{ fontSize: 13, color: "#1e293b" }}>{d.name}</strong>
            </div>
            <div><span style={{ background: "#f1f5f9", color: "#475569", borderRadius: 7, padding: "3px 9px", fontSize: 11, fontWeight: 700 }}>{d.code}</span></div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "#11182780", display: "grid", placeItems: "center", zIndex: 50, padding: 20 }} onMouseDown={closeModal}>
          <div style={{ background: "white", borderRadius: 18, width: "min(500px,100%)", padding: 28, boxShadow: "0 25px 70px #0005" }} onMouseDown={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "grid", gap: 14 }}>
              {[
                { key: "name", label: "Department Name", placeholder: "e.g. Engineering", required: true },
                { key: "code", label: "Department Code", placeholder: "e.g. ENG001", required: true },
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 22 }}>
              ...
              <button onClick={save} disabled={saving} style={{ border: 0, background: "linear-gradient(135deg,#4f46e5,#7c3aed)", color: "white", borderRadius: 10, padding: "10px 24px", fontWeight: 700, cursor: "pointer" }}>{saving ? "Saving…" : editing ? "Update" : "Add Department"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {delTarget && (
        <div style={{ position: "fixed", inset: 0, background: "#11182780", display: "grid", placeItems: "center", zIndex: 60, padding: 20 }} onMouseDown={cancelDelete}>
          <div style={{ background: "white", borderRadius: 18, width: "min(420px,100%)", padding: 28, boxShadow: "0 25px 70px #0005", textAlign: "center" }} onMouseDown={(e) => e.stopPropagation()}>
            ...
            {delStep === 1 ? (
              <>
                <h3 style={{ margin: "0 0 8px" }}>Delete Department?</h3>
                <p style={{ color: "#64748b", fontSize: 13, margin: "0 0 6px" }}>Delete <strong>{delTarget.name}</strong>?</p>
                {empCount(delTarget.name) > 0 && <p style={{ color: "#f59e0b", fontSize: 12, background: "#fffbeb", borderRadius: 8, padding: "8px 12px", margin: "0 0 10px" }}>⚠ {empCount(delTarget.name)} employee(s) assigned to this department.</p>}
              ...
              <>
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                  <button onClick={cancelDelete} style={{ border: "1px solid #e2e8f0", background: "white", color: "#475569", borderRadius: 10, padding: "10px 20px", fontWeight: 600, cursor: "pointer" }}>Cancel</button>
                  <button onClick={confirmDelete} style={{ border: 0, background: "linear-gradient(135deg,#dc2626,#b91c1c)", color: "white", borderRadius: 10, padding: "10px 22px", fontWeight: 700, cursor: "pointer" }}>Delete Department</button>
                </div>
              </>
            )}
          </div>
